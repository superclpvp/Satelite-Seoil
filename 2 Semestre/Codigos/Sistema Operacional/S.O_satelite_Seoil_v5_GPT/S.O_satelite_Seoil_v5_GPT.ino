...
void loop() {
  if (Em_Analise[1] == 0 && Em_Analise[2] == 0) {
    ...
    verificarTirarFotos();
    verificarEnviarFotos();
  }
}

void verificarTirarFotos() {
  int lines = 2;
  bool T = false;
  for (int i = 0; i < lines; i += 2) {
    String AnaliseSD = lerSD_linha("Tirar_Fotos", i);
    float latSD = AnaliseSD.toFloat();
    Serial.println(AnaliseSD + " " + Em_Analise[1]);
    if (Em_Analise[1] > latSD - 0.0001 && Em_Analise[1] < latSD + 0.0001) {
      T = true;
      break;
    }
  }
...
void verificarEnviarFotos() {
  ...
  for (int i = 0; i < lines; i += 2) {
    ...
    if (Em_Analise[2] > latSD - 0.0001 && Em_Analise[2] < latSD + 0.0001) {
      L = true;
      break;
    }
  }
